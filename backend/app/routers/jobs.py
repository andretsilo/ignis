...
async def create_job(
    ...
    job = await job_service.create_job(zip, image, entrypoint, db, current_user)
...
async def list_jobs(
    ...
    jobs = await job_service.list_jobs(db, current_user)
...
async def get_job(
    ...
    job = await job_service.get_job(job_id, db, current_user)
    return JobOut.model_validate(job)
